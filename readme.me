...
Spendwise is a Flutter mobile app for personal finance management, including auth, expense tracking, wallet balance control, statistics, and profile settings.
...
- Readable, structured code:
  - Features grouped by folder and responsibility.
  - Reusable components in `widgets/` and shared helpers in `utils/`.

### 2. Testing & Documentation (5 pts)
- Test cases included:
  - Unit tests and widget tests across models, providers, services, utils, and UI.
- Feature documentation:
  - Full feature list, architecture, code structure, run steps, and quality status are documented below.

## App Features (All Modules)
- Authentication:
  - Register with validation
  - Login with email/password
  - Google login flow
  - Persistent auth session using `SharedPreferences`
- Splash and startup:
  - Splash screen checks auth state and routes user to login or main app
- Home / Transactions:
  - View recent transactions
  - Add transaction
  - Edit transaction
  - Delete transaction (dialog + swipe)
  - Pull-to-refresh and pagination
  - Filter by category
- Statistics:
  - Daily and monthly spending summary
  - Category totals and visual breakdown
- Wallet:
  - Fetch wallet data
  - Top up wallet
  - Update wallet balance
  - Reset wallet balance
  - Last-month expense summary
...
- `providers/`:
  - business logic and state (`ChangeNotifier`)
- `services/`:
  - API/network calls using `Dio`
- `models/`:
  - data parsing and serialization
- `utils/`:
  - validators, constants, shared UI helpers
