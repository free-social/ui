...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final TextEditingController emailController = TextEditingController(text: "p@gmail.com");
  final TextEditingController passController = TextEditingController(text: "123!Pp");
  ...
  Widget build(BuildContext context) {
    ...
    final Color inputFill = isDark ? const Color(0xFF1E1E1E) : const Color(0xFFF9FAFB);
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                Text("Welcome Back", style: TextStyle(fontSize: 32, fontWeight: FontWeight.w800, color: textColor)),
                const SizedBox(height: 8),
                Text("Sign in to manage your expenses", style: TextStyle(color: subTextColor, fontSize: 16)),
                const SizedBox(height: 40),

                _buildLabel("Email Address", textColor),
                _buildTextField(emailController, "hello@example.com", Icons.email_outlined, false, null, inputFill, borderColor, hintColor, iconColor),
                
                const SizedBox(height: 25),

                _buildLabel("Password", textColor),
                _buildTextField(
                  passController, "Enter password", Icons.lock_outline, _obscurePassword, 
                  IconButton(
                    icon: Icon(_obscurePassword ? Icons.visibility_off : Icons.visibility, color: iconColor),
                    onPressed: () => setState(() => _obscurePassword = !_obscurePassword),
                  ),
                  inputFill, borderColor, hintColor, iconColor
                ),
                ...
                Row(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text("Or continue with", style: TextStyle(color: subTextColor, fontSize: 14)),
                    ),
                    Expanded(child: Divider(color: borderColor)),
                  ],
                ),
  ...
  Widget _buildLabel(String text, Color color) {
    return Align(alignment: Alignment.centerLeft, child: Padding(padding: const EdgeInsets.only(bottom: 8), child: Text(text, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: color))));
  }

  Widget _buildTextField(
    TextEditingController controller, String hint, IconData icon, bool obscure, Widget? suffix,
    Color fill, Color border, Color hintColor, Color iconColor
  ) {
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      await Provider.of<AuthProvider>(context, listen: false).login(emailController.text.trim(), passController.text);
